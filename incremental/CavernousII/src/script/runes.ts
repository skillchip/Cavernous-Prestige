...
const runes: Rune[] = [
	...
	new Rune(
		...
		0,
		() =>
			`This rune allows you to kill even hardy orthogonally ${
				(getRune("Wither").upgradeCount || 0) > 0 ? "or diagonally " : ""
			}adjacent plants.  Interact with it to charge it up - it takes ${
				(getRune("Wither").upgradeCount || 0) > 1 ? "1/" + 2 ** (getRune("Wither").upgradeCount - 1) : ""
			} as much Temporal to charge as the plants you're trying to kill would take to chop.<br>Requires:<br>{'0':'1 Salt<br>1 Iron Ore${
				getRealm("Verdant Realm").completed || getRealm("Verdant Realm").maxMult > 1e307 ? "" : "<br>1 Gold Nugget"
	...
	new Rune(
		...
		1000,
		() =>
			`Mine more resources with this rune.  After placing it,  interact with it to charge it up.  You'll receive +${
				1 + (getRune("Duplication").upgradeCount || 0) * 0.25
